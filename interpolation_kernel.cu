...
torch::Tensor trilinear_forward_cu(
    ...
    const dim3 threads(16, 16); //max=256
    const dim3 blocks((N + threads.x - 1) / threads.x, (F + threads.y - 1) / threads.y);

    AT_DISPATCH_FLOATING_TYPES(feats.type(), "trilinear_forward_cu",
    ([&] {
...
torch::Tensor trilinear_backward_cu(
    ...
    const int N = feats.size(0), F = feats.size(2);
    torch::Tensor dL_dfeats = torch::zeros({N, 8, F}, torch::dtype(torch::kFloat32).device(feats.device()));
    //复制输入的device与type
    // torch::Tensor feat_interp = torch::zeros({N, F}, feats.options());
    const dim3 threads(16, 16); //max=256
    const dim3 blocks((N + threads.x - 1) / threads.x, (F + threads.y - 1) / threads.y);
